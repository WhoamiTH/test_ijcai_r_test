...
for dataset in dataset_list:
    ...
    for dataset_index in range(1, 6):
        ...
        pos_result = []
        neg_result = []
        train_file_name = './test_{0}/standlization_data/{0}_std_train_{1}.csv'.format(dataset, dataset_index)
        ...
        pos_result.append(im.shape[0])
        neg_result.append(bm.shape[0])
        ...
        for i in range(10):
            ...
            pos_result.append(im.shape[0])
            neg_result.append(bm.shape[0])
        pos_str_list = list(map(str, pos_result))
        neg_str_list = list(map(str, neg_result))
        pos_str = '\t'.join(pos_str_list)
        neg_str = '\t'.join(neg_str_list)

        # print(train_file_name+' end\n\n\n')
        cur_result_str = pos_str + '\t' + neg_str
        print(cur_result_str)
        # dataset_result.append(cur_result_str)
    # result_ditc.append(dataset_result)
...
#     num_epochs -= dependency_model_epoch
# else:
